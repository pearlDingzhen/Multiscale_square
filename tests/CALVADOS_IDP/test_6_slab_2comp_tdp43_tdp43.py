#!/usr/bin/env python3
"""
Test 6: Dual-Component SLAB Simulation (TDP43_CTD + TDP43)
使用 config_idp_slab_2comp_tdp43_tdp43.yaml 进行混合组分 SLAB 相分离模拟测试。
"""
...
def test_config_loading():
    ...
    try:
        ...
        if config.components[0].name != "TDP43":
            print(f"✗ 期望 TDP43，实际 {config.components[0].name}")
            return None
        if config.components[1].name != "TDP43_CTD":
            print(f"✗ 期望 TDP43_CTD，实际 {config.components[1].name}")
            return None
        print(f"  ✓ 组件验证通过")
        
        # 验证组件类型
        if config.components[0].type != ComponentType.MDP:
            print(f"✗ 第一个组件应为 MDP，实际 {config.components[0].type}")
            return None
...
def test_config_validation(config):
    ...
    try:
        ...
        if errors:
            ...
            return False
        print(f"✓ 配置验证通过")
        print(f"  - TDP43: type={config.components[0].type.value}, nmol={config.components[0].nmol}")
        print(f"  - TDP43_CTD: type={config.components[1].type.value}, nmol={config.components[1].nmol}")
        return True
...
def test_fasta_and_pdb_loading(config):
    """测试 6.3: FASTA 和 PDB 文件加载"""
    print("\n" + "=" * 60)
    print("测试 6.3: FASTA 和 PDB 文件加载")
    print("=" * 60)
...
def test_slab_simulation_run(config):
    """测试 6.4: 运行双组分 SLAB 模拟 (使用 CGSimulator)"""
    print("\n" + "=" * 60)
    print("测试 6.4: 运行 SLAB 双组分模拟 (CGSimulator)")
    print("=" * 60)
    ...
    try:
        ...
        # 输出目录
        test_output_dir = Path(__file__).parent / "test6_output"
...
def main():
    """主测试函数"""
    print("=" * 60)
    print("双组分 SLAB 模拟测试 (TDP43_CTD + TDP43) - CGSimulator")
    print("=" * 60)
