...
# ============================================================
# Energy Analysis - Output individual force energies
# ============================================================
print("\n" + "=" * 60)
print("COCOMO Force Energy Analysis")
print("=" * 60)

# Force names based on their order in the system
force_names = [
    'HarmonicBondForce',         # Force 0
    'HarmonicAngleForce',        # Force 1
    'CMMotionRemover',           # Force 2
    'Electrostatic (Nonbonded)', # Force 3
    'Van der Waals (LJ)',        # Force 4
    'Cation-pi',                 # Force 5
    'Pi-pi',                     # Force 6
    'ElasticNetwork'             # Force 7
]

# Assign force groups (max 32)
for i, force in enumerate(system.getForces()):
    if i > 31:
        break
    force.setForceGroup(i)

# Create context and compute energies
# IMPORTANT: Reload positions from PDB file to ensure we use the exact coordinates
print(f"Loading positions from PDB file: {pdb_file}")
positions = PDBFile(pdb_file).positions
print(f"Number of positions loaded: {len(positions)}")

integrator = VerletIntegrator(0.001 * picoseconds)
platform = Platform.getPlatformByName(resources)
context = Context(system, integrator, platform)
context.setPositions(positions)
print("Positions set in context for energy calculation")

# Get energy for each force group
total_energy = 0.0
print(f"{'Force Name':<30} | {'Energy (kJ/mol)':<20}")
print("-" * 55)
...
for i in range(system.getNumForces()):
    ...
    total_energy += energy
    name = force_names[i] if i < len(force_names) else f'Force {i}'
    print(f"{name:<30} | {energy:>18.4f}")

print("-" * 55)
print(f"{'TOTAL POTENTIAL ENERGY':<30} | {total_energy:>18.4f}")
print("=" * 60)
