...
class COCOMO:
    ...
    def _add_elastic_network(self, system, top):
        ...
        for chain_id in sorted(atoms_by_chain.keys()):
            chain_atoms = atoms_by_chain[chain_id]
            # Convert chain identifier string to integer index for residues_by_chain
            # chain_id is like 'A', 'B', 'A1', 'B2', etc. from topology
            # residues_by_chain uses integers 1, 2, 3, etc.
            chain_int_id = self._get_chain_id_from_letter(chain_id)
