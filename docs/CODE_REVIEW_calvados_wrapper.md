...
**文件**: `multiscale2/src/calvados_wrapper.py`  
**审查日期**: 2024年12月30日  
**代码行数**: 630行
...
`calvados_wrapper.py` 是 multiscale2 项目中专门用于包装和运行 **CALVADOS** 粗粒化分子动力学模拟的核心模块。CALVADOS 是一种基于序列的粗粒化模型，主要用于模拟无序蛋白（IDPs）和折叠蛋白（MDPs）的相分离行为。
...
**外部依赖**:
- `multiscale2.extern.ms2_calvados.calvados.cfg`: Config, Components 类
- `multiscale2.extern.ms2_calvados.calvados.sim`: 模拟运行核心
